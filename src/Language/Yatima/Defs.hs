...
module Language.Yatima.Defs where
...
--                          <> (encodeString "1$body" <> encodeAnon bdy)
...
--    2 -> do
--      ctor <- decodeCtor "Anon" "0$ctor" ["Var", "Ref", "Lam"]
--      case ctor of
--        "Var" -> VarA <$> decodeField "Var" "1$indx" decodeInt
--        "Ref" -> RefA <$> decodeField "Ref" "1$link" decodeCID
--        "Lam" -> LamA <$> decodeField "Lam" "1$body" decodeAnon
--    3 -> do
--      ctor <- decodeCtor "Anon" "0$ctor" ["App"]
--      case ctor of
